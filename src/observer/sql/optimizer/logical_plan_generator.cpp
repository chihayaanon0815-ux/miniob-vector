...
RC LogicalPlanGenerator::create_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  // Add sort operator if ORDER BY is present
  unique_ptr<LogicalOperator> sort_oper;
  vector<unique_ptr<Expression>> &order_by_exprs = select_stmt->order_by();
  if (!order_by_exprs.empty()) {
    vector<unique_ptr<Expression>> sort_exprs;
    vector<bool>                   sort_is_asc;
    for (auto &expr : order_by_exprs) {
      sort_exprs.emplace_back(expr->copy());
    }
    sort_is_asc = select_stmt->order_by_is_asc();

    sort_oper = make_unique<SortLogicalOperator>(std::move(sort_exprs), std::move(sort_is_asc));
    if (*last_oper) {
  ...
  // Add limit operator if LIMIT is specified
  unique_ptr<LogicalOperator> limit_oper;
  int limit_num = select_stmt->limit_num();
  if (limit_num >= 0) {
    limit_oper = make_unique<LimitLogicalOperator>(limit_num);
    if (*last_oper) {
